...
export function GiftsSection({ data }: { data: GiftsData }) {
  ...
  return (
    <section id="gift_details" className="template-section py-12 px-4 max-w-4xl mx-auto border-b border-gray-200">
      ...
      <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 max-w-2xl mx-auto space-y-6">
        {data.giftNote && (
          <p className="text-sm text-gray-700 leading-relaxed text-center italic">{data.giftNote}</p>
        )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {options.map((opt) => (
                ...
                >
                  ...
                  {opt.accountName && (
                    <span className="text-gray-600 block text-xs">{opt.accountName}</span>
                  )}
                  {opt.accountNumber && (
                    <span className="font-mono text-xs font-semibold text-gray-800 block mt-1 bg-gray-50 py-1 px-2 rounded">
                      {opt.accountNumber}
                    </span>
                  )}
                  {opt.image?.url && (
