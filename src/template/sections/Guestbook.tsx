        ...
        <Reveal direction="up" distance={16}>
          <div className="text-center mb-10 sm:mb-12 space-y-2">
            <span className="text-role-subheading text-[var(--event-accent,#f59e0b)]">
              FOLIO // 13 &bull; WORDS OF BLESSING
            </span>
            ...
            <StaggerList staggerDelay={0.08} className="space-y-4 sm:space-y-5">
              {messages.map((msg, idx: number) => {
                ...
                return (
                  <div
                    key={msg.id || idx}
                    className="relative overflow-visible bg-[var(--event-surface)] p-6 sm:p-7 rounded-2xl border-2 border-[var(--event-border)] shadow-[var(--event-shadow-paper-md)] hover:shadow-[var(--event-shadow-paper-lg)] transition-all space-y-4 font-sans"
                  >
                    <div className="relative z-10 space-y-4">
                      <p className="text-[var(--event-text-main)] italic leading-relaxed text-base sm:text-lg font-sans">
                      ...
                      <div className="flex justify-between items-center text-xs text-[var(--event-text-muted)] pt-3 border-t border-[var(--event-border-subtle)] font-mono">
                        <span className="font-serif font-bold text-[var(--event-text-main)] text-base not-italic flex items-center gap-2">
                          <Heart className="w-3.5 h-3.5 text-[var(--event-primary)] fill-current" />
                          {msg.guestName || "Guest"}
                        </span>
