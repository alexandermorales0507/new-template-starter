...
export function TimelineSection({ data }: { data: TimelineData }) {
  ...
  return (
    <section
      id="timeline_program"
      className="template-section section-surface-ivory pattern-ledger-rule pattern-standard"
    >
          ...
          <StaggerList staggerDelay={0.08} className="space-y-4 sm:space-y-6">
            {data.items.map((item, idx: number) => (
              <div key={item.id || idx} className="relative group">
                ...
                <div className="absolute -left-[31px] sm:-left-[39px] top-1.5 w-4 h-4 bg-[var(--wedding-surface)] border-2 border-[var(--wedding-primary)] rotate-45 group-hover:bg-[var(--wedding-primary)] group-hover:rotate-90 transition-all duration-300 shadow-xs" />

                <div className="bg-[var(--wedding-surface)] p-5 sm:p-6 rounded-2xl border border-[var(--wedding-border)] shadow-xs hover:border-[var(--wedding-accent)]/60 transition-colors">
