...
export function CorrespondenceSheet({
  ...
  return (
    <div
      data-surface="light"
      className={cn(
        "correspondence-sheet relative mx-auto w-full max-w-3xl rounded-2xl border border-[var(--wedding-border)] bg-[var(--wedding-surface)] text-[var(--wedding-text)] p-8 sm:p-12 shadow-card text-left transition-all",
        className
      )}
      {...props}
    >
      ...
      {/* Sheet Content */}
      <div className="space-y-6 leading-relaxed text-[var(--wedding-text)] text-sm sm:text-base font-sans">
        {children}
      </div>
    </div>
  );
